...
namespace WalletConnectSharp.NEthereum.Client
{
    public class FallbackProvider : IClient
    {
        ...
        public Task<RpcRequestResponseBatch> SendBatchRequestAsync(RpcRequestResponseBatch rpcRequestResponseBatch)
        {
            ...
            if (rpcRequestResponseBatch.GetRpcRequests().Length > 0 &&
                ValidMethods.Contains(rpcRequestResponseBatch.GetRpcRequests()[0].Method))
            {
                return _signer.SendBatchRequestAsync(rpcRequestResponseBatch);
            }

            return _fallback.SendBatchRequestAsync(rpcRequestResponseBatch);
        }
